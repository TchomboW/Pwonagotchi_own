...
class BjornOrchestrator:
    ...
    async def switch_network(self, mode: str) -> bool:
        print(f"[Bjorn] {'\n🔄 LOG: Initiating physical switch to [{mode}]...'}")
        success, output = await self._execute(["network", mode])
        if success:
            self.active_interface = "wlan0" if mode == "wifi" else "eth0"
            print(f"[Bjron] ✅ LOG: Network transition successful. Current: {self.active_interface}")
            return True
        else:
...
async def main_test():
    orchestrator = BjornOrchestrator("/Users/tony/Pwonagotchi_own/rust_engine/target/release/pwon_core", "/Users/tony/Pwonagotchi_own/intelligence_accumulated.json")
